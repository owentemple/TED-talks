...
# --- Load data and fit classifier at startup ---
df = pd.read_csv(os.path.join(settings.PROCESSED_DIR, "all_with_liwc_segmented.csv"))

# Fit the persuasiveness classifier
...
print(f"Loaded {len(df)} talks, classifier trained.")
...
# Uses the startup-fitted model to classify new text
...
def analyze_text():
    ...
    single_df = find_similar_speaker(name)
